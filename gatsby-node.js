...
const mime = require('mime-types'); //TODO: gatsby default babel setup
...
async function fetchReadme(urlBuilder, path, reporter, fileNumber = 0) {
  ...
  try {
    ...
    if (e.response && e.response.status === 404 && README_FILES[nextFileNumber]) return fetchReadme(urlBuilder, path, reporter, nextFileNumber);else reporter.warn(`Unable to find readme for: ${path}.
...
async function getPackageReadme(pkg, reporter) {
  ...
  if (README_DOMAINS.hasOwnProperty(hostname)) {
    ...
    reporter.verbose(`Package "${name}" isn't hosted on Github, unable to fetch Readme yet (PRs welcome).`);
  }
}
  ...
  if (!query || !(query.name || query.type || query.tags)) {
    ...
    try {
      const {
        results: packages,
        total
      } = await getAllSearchResults(async () => packagist.search(query));
      ...
      for (pkg of packages) {
        const {
          file: readme,
          mimeType
        } = (await getPackageReadme(pkg, reporter)) || '';
        actions.createNode({ ...pkg,
          readme: readme,
